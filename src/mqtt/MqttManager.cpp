...
void MqttManager::onMessageReceived(const struct mosquitto_message *msg)
{
    if(mMediator)
    {
        ...
        {
            std::cout << "Received message with null topic" << std::endl;
            return;
        }
        ...
        {
            std::cout << "Received message with payload on topic " << msg->topic << std::endl;
    ...
    {
        std::cout << "Mediator not set, cannot handle message on topic " << msg->topic << std::endl;
    }
}
...
{
    {
        ...
        {
            std::cout << "Failed to initialize mosquitto library" << std::endl;
            return false;
        }
    }
    ...
    if(!mCaCertPath.empty())
    {
        ...
        {
            std::cout << "Failed to setup TLS: " << mosquitto_strerror(ret) << " (" << ret << ")" << std::endl;
    ...
    if(ret != MOSQ_ERR_SUCCESS)
    {
        std::cout << "Failed to connect to broker: " << mosquitto_strerror(ret) << " (" << ret << ")" << std::endl;
        return false;
    }
...
void MqttManager::start()
{
    ...
    if (ret != MOSQ_ERR_SUCCESS)
    {
        std::cout << "Error: failed to start mosquitto loop: " << ret << std::endl;
...
bool MqttManager::publishMessage(const std::string topic, const std::string message, const int qos, const bool retain, const mosquitto_property *properties)
{
    if (!mMosq)
    {
        std::cout << "Mosquitto instance not initialized" << std::endl;
        return false;
    }
    ...
    if (ret != MOSQ_ERR_SUCCESS)
    {
        std::cout << "Error: failed to publish message: " << ret << std::endl;
        mosquitto_disconnect_v5(mMosq, MQTT_RC_UNSPECIFIED, nullptr);
        return false;
    }
