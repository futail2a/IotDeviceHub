#include "BluezSdbusHandler.h"
#include <iostream>

bool BluezSdbusHandler::init()
{
    mConnection = sdbus::createSystemBusConnection();
    mBluezProxy = sdbus::createProxy(*mConnection, BLUEZ_SERVICE, HCI0_PATH);
};

void BluezSdbusHandler::terminate()
{
};

bool BluezSdbusHandler::startScan()
{
    if(!mConnection || !mBluezProxy)
    {
        std::cout << "BluezSdbusHandler is not initialized." << std::endl;
        return false;
    }
...
bool BluezSdbusHandler::stopScan()
{
    if(!mConnection || !mBluezProxy)
    {
        std::cout << "BluezSdbusHandler is not initialized." << std::endl;
...
void BluezSdbusHandler::setDevice(std::shared_ptr<BleDeviceHandler> device)
{
    if (device)
    {
...
std::weak_ptr<BleDeviceHandler> BluezSdbusHandler::findDeviceFromMac(std::string mac)
{
...
void BluezSdbusHandler::onPropertiesChanged(sdbus::Signal& signal, BluezSdbusHandler* handler)
{
...
void BluezSdbusHandler::connectDevices()
{
    for(auto device : mBleDeviceHandlers)
    {
        if(auto dev = device.lock())
...
void BluezSdbusHandler::sendCommand(const BleCommand& command)
{
    ...
    if (auto dev = device.lock())
    {
        ...
        try {
            ...
            {
                auto reply = charProxy->callMethod("ReadValue")
                    .onInterface(GATT_CHAR_1)
                    .withArguments(command.options);
...
void BluezSdbusHandler::onInterfacesAdded(const sdbus::Signal& signal)
{
